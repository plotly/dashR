...
for (i in seq_along(core)) {
  ...
  hasChildren <- "children" %in% names(props)
  header <- glue(
    ...
    children = if (hasChildren) "#' @param ... children of the component." else "",
    linebreak = "\n"
  )
  body <- glue(
  ...
  func <- glue(
    ...
    params = paste0(if (hasChildren) "..., ", paste(names(defaultValues), "=", defaultValues, collapse = ", ")),
    params2 = paste0(if (hasChildren) "children = assert_valid_children(...), ", paste(names(props), names(props), sep = "=", collapse = ", \n\t\t\t\t")),
    type = componentName,
    .open = "*{", .close = "}*"
  )
...
for (i in seq_along(html)) {
  ...
  props <- component$props
  if (!"children" %in% names(props)) stop("Thought they all had children", call. = FALSE)
