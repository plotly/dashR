...
ui <- htmlDiv(
  ...
  coreMarkdown(
    ...
    "",
    "One of the really cool things about dash & dasher is that",
    ...
    "by the community.",
    "",
    "## From React.js to dasher",
    "",
    "The dashTranspileR package provides a way to",
    ...
    "the dasher ecosystem. If you're already familiar with",
    "[transpiling to dash](https://plot.ly/dash/plugins), you'll find that",
    "transpiling to dasher is _very_ similar (in fact, if you've already followed",
    "that tutorial to create a dash (python) component, you can add dasher (R)",
    "support by doing `npm run install-local-r`).",
    "",
    "On a high level, transpiling from JS to R works like so:",
    "",
    "- Components in dasher are serialized as [JSON](www.json.org).",
    "To write a dasher compatible component, all of the properties",
    "of the component must be serializable as JSON. For example,",
    "JavaScript functions are not valid input arguments.",
    "",
    "- By annotating components with React docstrings, dasher extracts",
    ...
    "",
    "- dashTranspileR reads this JSON file and dynamically creates R functions that return",
    "objects that dasher understands. _These functions and their documentation are",
    ...
    "",
    "- At runtime, dasher crawls through an app's layout and finds which R package",
    "each component belongs to so that it can serve the necessary CSS/JS bundles to the front-end.",
    "",
    "- The layout is serialized as JSON, served to dasher's front-end",
    ...
    "## Step-by-step guide",
    "",
    "The dasher ecosystem includes a command-line tool for",
...
",
  ...
  coreMarkdown(
    ...
    "functional dash and/or dasher components. Located in the",
...
",
  ...
  coreMarkdown(
    "### Step 5 - View the component in a dasher app",
    "",
  ...
  coreMarkdown(
    ...
    "encouraged to change this filename (the dash/dasher component name derives from",
    ...
    "with these changes.",
    "",
    "5. Run `npm run install-local-r` and modify `inst/USAGE-R.md` to reflect dasher",
  ...
  coreMarkdown(
    "Remember that all dasher components are described declaratively through a set of",
    ...
    "interface to dasher, your component code must call",
    ...
    "with that property, and will send that new value to the appropriate dasher",
    "callback."
  ),
  ...
  coreMarkdown(
    ...
    "version specified in the package.json file. By convention, dasher components",
